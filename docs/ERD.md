...
Re-Echo 1차 MVP의 ERD는 다음 흐름을 안정적으로 지원하는 것을 목표로 한다.
...
이번 ERD는 `docs/PRD.md`와 `docs/Architecture.md`를 기준으로 작성하며,
MVP 범위 밖의 검색, 멘션, DM, AI 기능은 포함하지 않는다.
...
- 워크스페이스에는 `OWNER`, `ADMIN`, `MEMBER` 역할이 있다.
- 워크스페이스 참여 시 기본 채널 `#general`에 자동 참여한다.
- 채널은 공개 채널과 비공개 채널로 구분된다.
- 메시지는 실시간으로 송수신되며, 본인 메시지는 수정할 수 있다.
- 관리자는 모든 멤버의 메시지를 삭제할 수 있다.
- 파일 첨부 메타데이터는 애플리케이션 DB에 저장한다.
- 읽음 상태와 안 읽은 메시지 수는 채널 단위로 계산한다.
...
- User
  - Re-Echo 사용자 계정
- UserIdentity
  - 소셜 로그인 공급자와 사용자 계정의 연결
- Workspace
  - 협업의 최상위 공간
- WorkspaceMembership
  - 사용자와 워크스페이스의 소속 및 역할
- WorkspaceInviteLink
  - 워크스페이스 참여용 초대 링크
- Channel
  - 워크스페이스 내부 대화 공간
...
| --- | --- | --- | --- | --- | --- | --- |
| id | uuid | Y |  | N |  | 기본 키 |
| display_name | varchar(80) |  |  | N |  | 사용자 표시 이름 |
...
| provider_user_id | varchar(191) |  |  | N |  | 공급자 내부 사용자 식별자 |
| provider_email | varchar(320) |  |  | Y |  | 공급자 제공 이메일 |
| created_at | timestamptz |  |  | N | `now()` | 생성 시각 |

제약:

- `(provider, provider_user_id)` 유니크
- 한 identity는 반드시 하나의 user에 속한다
...
| banned_at | timestamptz |  |  | Y |  | 재초대 금지 처리 시각 |
| created_at | timestamptz |  |  | N | `now()` | 생성 시각 |
| updated_at | timestamptz |  |  | N | `now()` | 수정 시각 |
...
- 빈 문자열 메시지 금지
- 첨부만 있는 메시지를 허용할지 여부는 Open Issues로 남긴다
...
- 안 읽은 메시지 수는 저장값이 아니라 조회 시 계산하는 것을 기본 원칙으로 둔다

### 7.10 `file_objects`

설명:
외부 오브젝트 스토리지에 저장된 파일의 메타데이터다.
...
| storage_provider | varchar(30) |  |  | N |  | 기본값은 `R2` |
...
- 채널 참여는 사용자 직접 참조가 아니라 `workspace_membership`을 기준으로 연결한다.
- 메시지 작성자, 초대 링크 발급자, 파일 업로더는 모두 워크스페이스 맥락을 가지는 멤버십 기준으로 추적한다.
...
- 파일 메타데이터 등록 실패 시 `ORPHANED` 상태로 마킹 가능
- 정기 스케줄러가 고아 파일과 만료 대상 파일을 정리한다
- 메시지 또는 채널 삭제와 파일 물리 삭제의 동기화 방식은 배치 정리를 전제로 둔다
...
- 사용자 인증 공급자 정보는 `users`와 분리된 `user_identities`로 관리한다.
- 채널 참여는 사용자 계정이 아니라 워크스페이스 멤버십을 기준으로 연결한다.
- 메시지 작성자, 파일 업로더, 초대 발급자 같은 행위 주체는 워크스페이스 문맥을 보존하기 위해 멤버십 FK를 우선 사용한다.
- 읽음 상태는 메시지별 영수증 테이블이 아니라 채널별 마지막 읽은 메시지 기준으로 단순화한다.
- 파일은 R2에 저장하고 DB에는 메타데이터만 남긴다.
